...
const LiteZ = {
  ...
  ref(initial) {
      ...
      return {
          ...
          set value(newVal) { state.set('value', newVal); },
                      ...
                      // Add bind: support
                          ...
                          if (key === 'class') {
                              props['class'] = () => {
                                  ...
                                  return typeof classVal === 'string' ? classVal :
                                         Array.isArray(classVal) ? classVal.join(' ') :
                                         typeof classVal === 'object' ? Object.entries(classVal).filter(([, v]) => v).map(([k]) => k).join(' ') : '';
                              };
                          ...
                          directives['z-for'] = { item, list }; // Alias repeat to z-for
                      }
  ...
  component(name, { template, data = () => ({}), methods = {}, computed = {}, on = {}, props = {}, slots = {}, styles, scoped = false } = {}) {
              ...
              try {
                  ...
                  const applyStyles = () => {
                      if (styles && typeof document !== 'undefined') {
                          ...
                          const css = typeof styles === 'string' ? styles : Object.entries(styles)
                  ...
                  const { render: renderTemplate, scopeId } = typeof template === 'string' ? LiteZ.compileTemplate(template, context) : { render: template, scopeId: styleId };
                  ...
                  const instance = {
                      ...
                      on: {
                          ...
                          mount: () => { applyStyles(); on.mount?.(instanceState.get(), propsState.get()); },
                  ...
                  if (LiteZ.devtools.enabled) LiteZ.devtools.inspect(instance);
                  return instance;
              } catch (e) {
                  if (LiteZ.errorHandler) LiteZ.errorHandler(e);
                  else console.error(`Component creation error for ${name}:`, e);
                  return { template: () => LiteZ.h('div', {}, 'Error'), state: LiteZ.state({}), props: LiteZ.state({}), methods: {}, computed: {}, on: {}, dom: null };
              }
          }.bind(LiteZ)
      };
      ...
      if (typeof customElements !== 'undefined' && !customElements.get(kebabName)) {
          customElements.define(kebabName, class extends HTMLElement {
              constructor() {
                  super();
      ...
      try {
          ...
          if (!this.updateScheduled) {
              ...
              requestAnimationFrame(() => {
                  this.updateQueue.forEach(({ parent, newNode, oldNode, index }) =>
                      this.update(parent, newNode, oldNode, index));
  ...
  router(routes, target = '#app', middlewares = []) {
      ...
      try {
          ...
          const showRoute = () => {
              ...
              const compDef = this.components[route.component] || this.components['NotFound'] || {
                  createInstance: () => ({
              ...
              try {
                  ...
                  container._component = comp;
                  this.applyDirectives(container, comp.state);
                  if (this.devtools.enabled) this.devtools.inspect(comp);
                  if (this.devtools.enabled && window.__LITEZ_DEVTOOLS__) {
                      window.__LITEZ_DEVTOOLS__.trackRoute(path, route);
                  }
              } catch (err) {
  ...
  lazy(name, loader) {
      try {
          const placeholder = this.components['lite-suspense']?.createInstance() || {
  ...
  emit(event, data) {
      try {
          ...
          if (this.devtools.enabled) this.devtools.logEvent(event, data);
      } catch (e) {
          if (this.errorHandler) this.errorHandler(e);
 ...
 // ... (Previous LiteZ code up to devtools remains unchanged)
 ...
 devtools: {
  ...
  history: [],
  historyIndex: -1,
  performance: { renderTimes: [], updates: [] },
  highlighted: null,
  plugins: [],
  updateScheduled: false,
  init() {
      if (this.enabled && typeof window !== 'undefined') {
          if (!window.__LITEZ_DEVTOOLS__) {
              window.__LITEZ_DEVTOOLS__ = {
                  ...
                  stateDiffs: [],
                  register: (litez) => {
                      LiteZ.devtools.ui = LiteZ.devtools.createUI();
                      document.body.appendChild(LiteZ.devtools.ui);
                      LiteZ.devtools.setupHighlighting();
                  },
                  inspect: (data) => {
                      const start = performance.now();
                      const shallowState = Object.keys(data.state.get()).reduce((acc, key) => {
                          acc[key] = data.state.get()[key];
                          return acc;
                      }, {});
                      const oldState = window.__LITEZ_DEVTOOLS__.components.get(data.name)?.state;
                      window.__LITEZ_DEVTOOLS__.components.set(data.name, { ...data, state: shallowState });
                      if (oldState) {
                          const diff = LiteZ.devtools.computeStateDiff(oldState, shallowState);
                          window.__LITEZ_DEVTOOLS__.stateDiffs.push({ name: data.name, diff, time: Date.now() });
                      }
                      LiteZ.devtools.history.push({ time: Date.now(), state: shallowState });
                      LiteZ.devtools.historyIndex++;
                      if (LiteZ.devtools.history.length > 50) LiteZ.devtools.history.shift();
                      LiteZ.devtools.performance.renderTimes.push({ name: data.name, time: performance.now() - start });
                      LiteZ.devtools.updateUI();
                      LiteZ.devtools.plugins.forEach(plugin => plugin.onInspect?.(data));
                  },
                  emit: (type, data) => {
                      window.__LITEZ_DEVTOOLS__.events.push({ type, data, time: Date.now() });
                      LiteZ.devtools.updateUI();
                      LiteZ.devtools.plugins.forEach(plugin => plugin.onEvent?.(type, data));
                  },
                  trackRoute: (path, route) => {
                      window.__LITEZ_DEVTOOLS__.routes.push({ path, route, time: Date.now() });
                      LiteZ.devtools.updateUI();
                      LiteZ.devtools.plugins.forEach(plugin => plugin.onRoute?.(path, route));
                  },
                  trackSource: (name, source) => {
                      window.__LITEZ_DEVTOOLS__.sources.set(name, source);
                  }
              };
          }
          window.__LITEZ_DEVTOOLS__.register(LiteZ);
      }
  },
  installPlugin(plugin) {
      this.plugins.push(plugin);
      if (this.ui) plugin.onInit?.(this);
  },
  ...
  createUI() {
      ...
      ui.id = 'litez-devtools';
      ui.style.cssText = `
          position: fixed; bottom: 10px; right: 10px; width: 450px; height: 650px;
          background: #ffffff; border-radius: 12px; box-shadow: 0 6px 16px rgba(0,0,0,0.2);
          overflow: hidden; z-index: 9999; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
          display: flex; flex-direction: column; transition: all 0.3s ease;
      `;
      ui.innerHTML = `
          <div style="background: #1a2a3a; color: #ecf0f1; padding: 12px 16px; font-size: 16px; font-weight: 600; border-bottom: 1px solid #34495e; cursor: move;" id="header">
              LiteZ DevTools
          </div>
          <div style="flex: 1; padding: 16px; overflow-y: auto; background: #f7f9fb;" id="content">
              <div id="component-tree" style="margin-bottom: 20px;"></div>
              <div id="event-log" style="margin-bottom: 20px;"></div>
              <div id="router-inspection" style="margin-bottom: 20px;"></div>
              <div id="performance" style="margin-bottom: 20px;"></div>
              <div id="state-diff" style="margin-bottom: 20px;"></div>
              <div id="source-link" style="margin-bottom: 20px;"></div>
          </div>
          <div style="padding: 12px; background: #ecf0f1; border-top: 1px solid #dcdcdc; display: flex; justify-content: space-between;">
              <button id="prev-state" style="padding: 6px 12px; border: none; background: #3498db; color: white; border-radius: 4px; cursor: pointer; font-size: 12px; transition: background 0.2s;">Previous</button>
              <button id="next-state" style="padding: 6px 12px; border: none; background: #3498db; color: white; border-radius: 4px; cursor: pointer; font-size: 12px; transition: background 0.2s;">Next</button>
          </div>
      `;
      ...
      header.onmousedown = (e) => {
          isDragging = true;
          startX = e.clientX - parseInt(ui.style.right || '10', 10);
          startY = e.clientY - parseInt(ui.style.bottom || '10', 10);
      };
      document.onmousemove = (e) => {
          if (isDragging) {
              ui.style.right = `${Math.max(0, startX - e.clientX)}px`;
              ui.style.bottom = `${Math.max(0, startY - e.clientY)}px`;
          }
      };
      document.onmouseup = () => { isDragging = false; };

      ui.querySelector('#prev-state').onclick = () => LiteZ.devtools.travel(-1);
      ui.querySelector('#next-state').onclick = () => LiteZ.devtools.travel(1);

      return ui;
  },
  setupHighlighting() {
      document.addEventListener('mouseover', (e) => {
          const el = e.target.closest('[data-litez-component]');
          if (el && el !== LiteZ.devtools.highlighted) {
              if (LiteZ.devtools.highlighted) LiteZ.devtools.highlighted.style.outline = '';
              el.style.outline = '2px solid #3498db';
              LiteZ.devtools.highlighted = el;
          }
      });
      document.addEventListener('mouseout', (e) => {
          if (LiteZ.devtools.highlighted && !e.target.closest('[data-litez-component]')) {
              LiteZ.devtools.highlighted.style.outline = '';
              LiteZ.devtools.highlighted = null;
          }
      });
  },
  updateUI() {
      if (!this.ui || !window.__LITEZ_DEVTOOLS__) return;
      try {
          const content = this.ui.querySelector('#content');
          if (!content) return;

          const tree = this.ui.querySelector('#component-tree');
          tree.innerHTML = '<h3 style="font-size: 14px; color: #2c3e50; margin-bottom: 8px;">Component Tree</h3>' + 
              Array.from(window.__LITEZ_DEVTOOLS__.components.entries())
                  .map(([name, comp]) => `<div style="padding: 4px 8px; background: #ecf0f1; border-radius: 4px; margin: 4px 0; font-size: 12px;">${name}</div>`)
                  .join('');

          const events = this.ui.querySelector('#event-log');
          events.innerHTML = '<h3 style="font-size: 14px; color: #2c3e50; margin-bottom: 8px;">Event Log</h3>' + 
              window.__LITEZ_DEVTOOLS__.events.slice(-5).map(ev => 
                  `<div style="padding: 4px 8px; background: #ecf0f1; border-radius: 4px; margin: 4px 0; font-size: 12px;">${ev.type}: ${JSON.stringify(ev.data)}</div>`
              ).join('');

          const routes = this.ui.querySelector('#router-inspection');
          routes.innerHTML = '<h3 style="font-size: 14px; color: #2c3e50; margin-bottom: 8px;">Routes</h3>' + 
              window.__LITEZ_DEVTOOLS__.routes.slice(-5).map(r => 
                  `<div style="padding: 4px 8px; background: #ecf0f1; border-radius: 4px; margin: 4px 0; font-size: 12px;">${r.path}</div>`
              ).join('');

          const perf = this.ui.querySelector('#performance');
          perf.innerHTML = '<h3 style="font-size: 14px; color: #2c3e50; margin-bottom: 8px;">Performance</h3>' + 
              this.performance.renderTimes.slice(-5).map(p => 
                  `<div style="padding: 4px 8px; background: #ecf0f1; border-radius: 4px; margin: 4px 0; font-size: 12px;">${p.name}: ${p.time.toFixed(2)}ms</div>`
              ).join('');

          const diff = this.ui.querySelector('#state-diff');
          diff.innerHTML = '<h3 style="font-size: 14px; color: #2c3e50; margin-bottom: 8px;">State Diff</h3>' + 
              window.__LITEZ_DEVTOOLS__.stateDiffs.slice(-5).map(d => 
                  `<div style="padding: 4px 8px; background: #ecf0f1; border-radius: 4px; margin: 4px 0; font-size: 12px;">${d.name}: ${JSON.stringify(d.diff)}</div>`
              ).join('');

          const sources = this.ui.querySelector('#source-link');
          sources.innerHTML = '<h3 style="font-size: 14px; color: #2c3e50; margin-bottom: 8px;">Sources</h3>' + 
              Array.from(window.__LITEZ_DEVTOOLS__.sources.entries())
                  .map(([name]) => `<div style="padding: 4px 8px; background: #ecf0f1; border-radius: 4px; margin: 4px 0; font-size: 12px;">${name}</div>`)
                  .join('');
      } catch (e) {
          if (LiteZ.errorHandler) LiteZ.errorHandler(e);
          else console.error('DevTools UI update error:', e);
      }
  },
  travel(step) {
      const newIndex = Math.max(-1, Math.min(this.history.length - 1, this.historyIndex + step));
      if (newIndex === this.historyIndex || newIndex < 0 || newIndex >= this.history.length) return;

      this.historyIndex = newIndex;
      const state = this.history[this.historyIndex].state;
      window.__LITEZ_DEVTOOLS__.components.forEach((comp, name) => {
          if (state[name]) comp.state.set(state[name]);
      });
      this.updateUI();
  },
  logEvent(type, data) {
      if (this.enabled && window.__LITEZ_DEVTOOLS__) {
          window.__LITEZ_DEVTOOLS__.emit(type, data);
      }
  }
},
...
createApp({ template, data = () => ({}), methods = {}, computed = {}, on = {}, props = {}, slots = {} }) {
  try {
      ...
      const { render: renderTemplate, scopeId } = typeof template === 'string' ? this.compileTemplate(template, context) : { render: template, scopeId: `app-${Math.random().toString(36).slice(2)}` };
      ...
      const app = {
          ...
          mount(selector) {
              ...
              if (this.devtools.enabled) this.devtools.inspect({ name: 'App', ...this });
              return this;
          }
      };
...
LiteZ.devtools.enabled = true; // Enable devtools by default in browser
LiteZ.devtools.init();
window.LiteZ = LiteZ;
}
